...
export default function Productcarddetails() {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '60vh', gap: '15px' }}>
  ...
  const handleBuyNow = () => {
    ...
    if (!user) {
      Swal.fire({ toast: true, position: 'top', icon: 'warning', title: 'Please login first', showConfirmButton: false, timer: 2000, timerProgressBar: true });
      navigate("/login");
      return;
    }
    ...
    navigate("/checkout", { state: { cartItems, totalAmount: product.price, user } });
  };
  ...
  function renderStars(rating) {
    ...
    for (let i = 0; i < fullStars; i++) { stars.push(<span key={`f-${i}`} className="star star-full">★</span>); }
    if (hasHalfStar) { stars.push(<span key="half" className="star star-half">★</span>); }
    const emptyStars = 5 - Math.ceil(rating || 0);
    for (let i = 0; i < emptyStars; i++) { stars.push(<span key={`e-${i}`} className="star star-empty">★</span>); }
    return stars;
  }
    ...
    <>
      <div className="product-detail-container">
        ...
        <div className="image-section">
          ...
          {/* 🔴 FIXED: ADDITIONAL IMAGES THUMBNAILS ROW */}
          {product.images && product.images.length > 0 && (
            <div className="thumbnail-images-container" style={{ display: 'flex', gap: '10px', marginTop: '15px', overflowX: 'auto', paddingBottom: '5px' }}>
              {/* Pehle main image ka thumbnail */}
              <div 
                className={`thumbnail-item ${selectedImage === product.img ? 'active-thumb' : ''}`} 
                onClick={() => setSelectedImage(product.img)}
                style={{ width: '70px', height: '70px', border: selectedImage === product.img ? '2px solid #3498db' : '1px solid #ddd', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer' }}
              >
                <img src={product.img} alt="main-thumb" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
              
              {/* Baki additional images */}
              {product.images.map((imgUrl, idx) => (
                <div 
                  key={idx} 
                  className={`thumbnail-item ${selectedImage === imgUrl ? 'active-thumb' : ''}`} 
                  onClick={() => setSelectedImage(imgUrl)}
                  style={{ width: '70px', height: '70px', border: selectedImage === imgUrl ? '2px solid #3498db' : '1px solid #ddd', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer' }}
                >
        ...
        <div className="info-section">
          ...
          <p className="price">
            ...
            {discountLabel && <span className="discount">{discountLabel}</span>}
          </p>
          ...
          {product.specifications && Object.keys(product.specifications).length > 0 && (
            <div className="product-specifications-box" style={{ marginTop: '20px', padding: '15px', background: '#f9f9f9', borderRadius: '8px', border: '1px solid #eee' }}>
              <h4 style={{ margin: '0 0 10px 0', color: '#333', fontSize: '16px', borderBottom: '1px solid #ddd', paddingBottom: '5px' }}>
      ...
      {((reviewStats && reviewStats.totalReviews > 0) || (!reviewStats || reviewStats.totalReviews === 0)) && (
        <div className="reviews-section">
          ...
          {reviews.length === 0 && <p style={{ color: '#666' }}>No reviews yet. Be the first to review this product!</p>}
      ...
      {showReviewModal && (
        <div className="review-modal-overlay" onClick={() => setShowReviewModal(false)}>
          <div className="review-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              if (rating === 0) { Swal.fire({ icon: 'warning', title: 'Rating Required', text: 'Please select a star rating.' }); return; }
              if (reviewText.trim().length < 10) { Swal.fire({ icon: 'warning', title: 'Review Too Short', text: 'Please write a review with at least 10 characters.' }); return; }
              
              try {
                await submitReview({ productId: product._id, orderId: eligibleOrderId, rating, review: reviewText });
                Swal.fire({ icon: 'success', title: 'Thank You!', text: 'Review submitted successfully', timer: 2500, showConfirmButton: false });
                setShowReviewModal(false);
                ...
                console.error('Submit review error:', err);
                Swal.fire({ icon: 'error', title: 'Oops...', text: err.response?.data?.message || 'Failed to submit review' });
              }
            }}>
              <div className="rating-section" style={{ padding: 20 }}>
                <label>Your Rating *</label>
